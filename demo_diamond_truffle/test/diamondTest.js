...
contract('DiamondTest', async (accounts) => {
  ...
  it('should test1 function call MyAddress', async () => {
    const test1FacetDiamond = new web3.eth.Contract(Test1Facet.abi, diamond.address)
    // await test1FacetDiamond.methods.test1Func1().send({ from: web3.eth.defaultAccount, gas: 1000000 })
    myAddress = await test1FacetDiamond.methods.test1Func2().call()
    assert.equal(
      zeroAddress,
      myAddress
    )
  ...
  it('should test1 function call MyNum', async () => {
    const test1FacetDiamond = new web3.eth.Contract(Test1Facet.abi, diamond.address)
    await test1FacetDiamond.methods.test1Func3(1234).send({ from: web3.eth.defaultAccount, gas: 1000000 })
    myNum = await test1FacetDiamond.methods.test1Func4().call()
    assert.equal(
      1234,
      myNum
    )
