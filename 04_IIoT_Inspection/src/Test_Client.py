...
class queryTxsThread(threading.Thread):
	...
	def run(self):
		...
		ret_msg=queryDataRef(_address, _id)
		logger.info(ret_msg)
...
def query_txs(args):
	...
	for idx in range(thread_count):
		...
		p_thread = queryTxsThread( [op_status, target_address, tokenId+idx] )
...
def queryDataRef(target_address, token_id):

	api_url = "http://" + target_address + "/NFT/api/v1.0/getDataRef"
	headers = {'Content-Type' : 'application/json'}
...
def define_and_get_arguments(args=sys.argv[1:]):
	...
	parser.add_argument("--test_func", type=int, default=0, 
	                    ...
	                    0-query main account and data tokens, \
	                    1-Services_test")

	parser.add_argument("--op_status", type=int, default=0, 
	                    help="operation status: based on app")

	parser.add_argument("--target_address", type=str, default="127.0.0.1:8180", 
...
if __name__ == "__main__":
	...
	if(args.test_func==1): 
		ret_msg=queryDataRef(args.target_address, args.id)
		logger.info(ret_msg)
		...
		for i in range(args.tx_round):
			...
			FileUtil.save_testlog('test_results', 'getDataRef_client.log', str_time_exec)
