...
def query_token(tokenId):
	...
	data_ac = token_dataAC.get_DataRef(tokenId)
...
def audit_data(token_id):
	...
	## build a Merkle tree from data_mac list
	merkle_tree = MerkleTree(data_mac)

	## get merkle root
	re_mkt_root = merkle_tree.get_root()
...
def test_Data(tokenId, ls_args):
	...
	print('Token {} DataRef_setup.'.format(tokenId))
	receipt = token_dataAC.DataRef_setup(tokenId, ls_args[0], ls_args[1], ls_args[2])
...
if __name__ == "__main__":
	...
	if(args.test_func==1):
		...
		for i in range(args.tx_round):
			...
			start_time=time.time()
			## add prod_id at the head of list
			prod_id = "prod-" + str(token_id)
			ls_reference=[prod_id]+ls_parameters

			# print(ls_reference)

			receipt = test_Data(token_id, ls_reference)
			if(receipt!=None):
				logger.info("exec_time: {} sec   gasUsed: {}".format( format( time.time()-start_time, '.3f'), receipt['gasUsed'] ))
				ls_time_exec.append( format( time.time()-start_time, '.3f') )
				str_time_exec=" ".join(ls_time_exec)
		...
		print("DataRef total supply: %d" %(total_supply))
